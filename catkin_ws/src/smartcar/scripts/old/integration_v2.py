...
def initial_parameters():
    ...
    srcps_ramp = np.float32([[(211, 255), (114, 387), (615, 387), (477, 255)]])
        ...
        while not rospy.is_shutdown():
            ...
            if ret == True:
                if(False):#not starter):
                    ...
                    hsv_Blur = cv2.morphologyEx(hsv_Blur, cv2.MORPH_CLOSE, kernel, iterations = 1)
                    black_lower=np.array([26,43,140])
                    black_upper=np.array([34,255,255])
                    black_mask=cv2.inRange(hsv_Blur,black_lower,black_upper)
                    black_target = cv2.bitwise_and(img, img, mask=black_mask)
                    black_gray = cv2.cvtColor(black_target,cv2.COLOR_BGR2GRAY)
                    ret,black_binary = cv2.threshold(black_mask,threshold_value,255,cv2.THRESH_BINARY)
                    ...
                    binary_warped =  perspectiveTrans(black_binary)
                    cv2.imshow('origin_thr',origin_thr)
                    cv2.imshow('black_binary',black_binary)
...
if __name__ == '__main__':
    ...
    initial_parameters()
    #距离映射
    x_cmPerPixel = 90/400.0
    y_cmPerPixel = 90/400.0
    roadWidth = 90.0 / x_cmPerPixel #80.0
    y_offset = 0.0 #cm
    #
