...
public:
  ...
  double dist2step = 923.445; //走0.01m的步数
  double odom_perimeter = 0.5338;//m 轮胎周长,非精确测量，暂时不用
  double odom_wheel = 0.30; //m 后轮间距
  double odom_front_overhang = 0.30; //m 前悬长度
  double odom_rear_overhang = 0.30; //m 后悬长度
  double middle_angle = 2000.0;
  odom();
  ~odom();
  float updateOdom(int step,int angular,double dt);
...
float odom::updateOdom(int step,int angular,double dt){
    /**
    ...
    double differ = middle_angle - angular;
    angular = -(differ/652)*180/3.14;
    ROS_INFO_STREAM("angular: "<<angular);    
    //计算步数变化量
    int delta_step = step - odom_step;
    ROS_INFO_STREAM("delta_step: "<<delta_step);
    float dist = (delta_step/dist2step)/100;
    ROS_INFO_STREAM("dist: "<<dist);    
    odom_speed = dist/dt;
    ROS_INFO_STREAM("odom_speed: "<<odom_speed);
    //计算滑移角beta
    ...
    ROS_INFO_STREAM("odom_x: "<<odom_x);
    ROS_INFO_STREAM("odom_y: "<<odom_y);
...
public:
  ...
  int middle_angle = 2000;///舵机正中位置为2000
...
int main(int argc, char** argv)
{
  ...
  float sum_dist = 0.0;
  ROS_INFO_STREAM("last_time: "<<last_time);
  while(ros::ok()){
      if(ser.available())
      {
            ...
            int angular = atoi(sub[2].c_str());
            ROS_INFO_STREAM("step: "<<step<<" angular: "<<angular); 
            begin_time = ros::Time::now();
            double dt = (begin_time - last_time).toSec();
            //ROS_INFO_STREAM("begin_time: "<<begin_time);
            ROS_INFO_STREAM("dt: "<<dt);
            if(step != 0 && las_step != step){
              sum_dist += myodom.updateOdom(step,angular,dt);
            }
            /*
            else{
              last_time = begin_time;
            }*/
            ROS_INFO_STREAM("sum_dist: "<<sum_dist);
            las_step = step;
            last_time = begin_time;
      }
      ros::spinOnce();
